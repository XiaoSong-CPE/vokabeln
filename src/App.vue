...
watch(mask, (values) => {
  ...
  <style>
    tr td.table-stern {
      color: ${values.Stern ? 'transparent' : 'var(--n-td-text-color)'} !important;
    }
    tr td.table-wort {
      color: ${values.Wort ? 'transparent' : 'var(--n-td-text-color)'} !important;
    }
    tr td.table-wortart {
      color: ${values.Wortart ? 'transparent' : 'var(--n-td-text-color)'} !important;
    }
    tr td.table-chinesisch {
      color: ${values.Chinesisch ? 'transparent' : 'var(--n-td-text-color)'} !important;
    }
    tr td.table-beispielsatz {
      color: ${values.Beispielsatz ? 'transparent' : 'var(--n-td-text-color)'} !important;
    }
    tr:hover td {
...
const observer = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.attributeName === 'class') {
      theme.value = targetNode.classList.contains('dark') ? darkTheme : undefined
    }
  })
})
  ...
  <n-config-provider :theme="theme" :locale="deDE" :date-locale="dateDeDE">
    ...
    <n-form>
      ...
      <n-form-item label="选择单词表">
        ...
        <n-checkbox v-model:checked="Teil.Intentionen"> Intentionen </n-checkbox>
        <n-checkbox v-model:checked="Teil.Hörverstehen"> Hörverstehen </n-checkbox>
        <n-checkbox v-model:checked="Teil.Leseverstehen"> Leseverstehen </n-checkbox>
      </n-form-item>
      ...
      <n-form-item label="遮住……">
        ...
        <n-checkbox v-model:checked="mask.Beispielsatz">Beispielsatz</n-checkbox>
      </n-form-item>
    </n-form>
    ...
    <div v-if="vokabeln?.Einführung && Teil.Einführung">
      <h3>Einführung</h3>
      <n-data-table :columns="columns" :data="vokabeln?.Einführung" :row-key="rowKey" />
    </div>

    <!-- Text -->
    <div v-if="vokabeln?.Text && Teil.Text">
      <h3>Text</h3>
      <n-data-table :columns="columns" :data="vokabeln?.Text" :row-key="rowKey" />
    </div>

    <!-- Übungen -->
    <div v-if="vokabeln?.Übungen && Teil.Übungen">
      <h3>Übungen</h3>
      <n-data-table :columns="columns" :data="vokabeln?.Übungen" :row-key="rowKey" />
    </div>

    <!-- Intentionen -->
    <div v-if="vokabeln?.Intentionen && Teil.Intentionen">
      <h3>Intentionen</h3>
      <n-data-table :columns="columns" :data="vokabeln?.Intentionen" :row-key="rowKey" />
    </div>

    <!-- Hörverstehen -->
    <div v-if="vokabeln?.Hörverstehen && Teil.Hörverstehen">
      <h3>Hörverstehen</h3>
      <n-data-table :columns="columns" :data="vokabeln?.Hörverstehen" :row-key="rowKey" />
    </div>

    <!-- Leseverstehen -->
    <div v-if="vokabeln?.Leseverstehen && Teil.Leseverstehen">
      <h3>Leseverstehen</h3>
      <n-data-table :columns="columns" :data="vokabeln?.Leseverstehen" :row-key="rowKey" />
    </div>
  </n-config-provider>
</template>
